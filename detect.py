...
center_prev = np.array([[None,None],[None,None],[None,None],[None,None],[None,None],[None,None],[None,None]])
total_dist = np.array([0, 0, 0, 0, 0, 0, 0])
pix_2_cm = 0
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det) < 8 and len(det):
                blurred = cv2.GaussianBlur(im0, (15, 15), 0, None)
                #converted_img = cv2.cvtColor(blurred, cv2.COLOR_GRAY2BGR)
                ...
                try:
                    ...
                    print("No se pudo actualizar escala")
                ...
                for *xyxy, conf, cls in reversed(det):
                  ...
                  try:
                    dxdy = np.sqrt((center_point[0]-center_prev[cont][0])**2 + (center_point[1]-center_prev[cont][1])**2)
                    if(dxdy<20):
                      total_dist[cont] += dxdy*pix_2_cm
                  except:
                    print("No hay info previa del pato")
                  if(cont < len(det)):
                    center_prev[cont][0] = center_point[0] 
                    center_prev[cont][1] = center_point[1] 
                    #print("La distancia del pato " + str(cont) + " es: " + str(total_dist[cont]) + " cm")
                    text_coord = cv2.putText(im0,("Pato "+ str(cont) + ": "+ str(total_dist[cont])+" cm"),(50,40+cont*20),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0))
